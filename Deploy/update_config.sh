...
# "m1:$private_host_ip","m2:$m2","m3:$m3","m4:$m4"
# sed -i 's/"m1: 127.0.0.1"/"m1:$private_host_ip","m2:$m2","m3:$m3","m4:$m4"/g' install_conf.json
sed -i 's@home@'"data"'@' install_conf.json
sed -i 's@sudoUserName@'"ubuntu"'@' install_conf.json
sed -i 's@/path/to/my_key.pem_rsa@'"/data/tigergraph/blacree"'@' install_conf.json
